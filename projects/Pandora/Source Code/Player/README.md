...
階層型有限ステートマシン(HFSM)のデザインパターンを参考に作成。アクションゲームのプレイヤーはユーザーからの入力、環境(地面にいるか、壁に触れているかなど)、ダメージ被弾など様々な要素を考慮しながらシステムを構築しなくてはならないので一番時間がかかった。基本的には中野陽(KOMIKURU)が作成。ただし、魔法攻撃に関連する部分はサークルの後輩(Silver)と共同で開発。
